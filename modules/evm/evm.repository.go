...
func (r repository) GetSupportedCryptoCurrencies(ctx context.Context, chainID int64, addresses []string) ([]*models.CryptoCurrency, error) {
	var jet_addresses []jet.Expression
	for _, address := range addresses {
		jet_addresses = append(jet_addresses, jet.String(address))
	}
	...
	FROM(tbl).
	WHERE(tbl.ChainID.EQ(jet.Int64(chainID)).AND(tbl.ContractAddress.IN(jet_addresses...)))
