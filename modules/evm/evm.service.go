...
type EvmService interface {
	...
	UpdateChainState(ctx context.Context, chainID int64, chainStateDto *dto.UpdateChainStateDto) error
	IsReorgDetected(headers []*types.Header, latest_checked_block_hash ethcommon.Hash) (bool, *int)
...
func (s *service) GetChainState(ctx context.Context, chainID int64) (*models.ChainState, error) {
	return s.repository.GetLatestChainState(ctx, chainID)
}

func (s *service) SearchByAddresses(ctx context.Context, chainID int64, addresses []string) ([]*models.Address, error) {
	return s.repository.GetWalletAddresses(ctx, chainID, addresses)
}

func (s *service) GetSupportedContracts(ctx context.Context, chainID int64, contracts []string) ([]*models.CryptoCurrency, error) {
	return s.repository.GetSupportedCryptoCurrencies(ctx, chainID, contracts)
}

func (s *service) CreateBlocks(ctx context.Context, chainID int64, headers []*types.Header) error {
	return s.repository.CreateBlocks(ctx, chainID, func(blocks *[]*models.Block) error {
		for _, h := range headers {
			*blocks = append(*blocks, &models.Block{
				BlockNumber: h.Number.String(),
				BlockHash: h.Hash().String(),
				BlockParentHash: h.ParentHash.String(),
				ChainID: chainID,
			})
		}
		return nil
...
func (s *service) UpdateChainState(ctx context.Context, chainID int64, chainStateDto *dto.UpdateChainStateDto) error {
	return s.repository.UpdateChainState(ctx, chainID, func(cs *models.ChainState) error {
		cs.ChainID = chainID
		cs.LastBlockNumber = &chainStateDto.LastBlockNumber
		cs.LastBlockHash = &chainStateDto.LastBlockHash
...
*/
func (s *service) IsReorgDetected(headers []*types.Header, latest_checked_block_hash ethcommon.Hash) (bool, *int) {
	// for this case we have to search on database to find ancestor block
	fmt.Println(latest_checked_block_hash)
	if latest_checked_block_hash != headers[0].ParentHash {
		current := 0
		return true, &current
	}

	for current := 1; current < len(headers); current++ {
		// previous := current - 1
		if headers[current].ParentHash != headers[current - 1].Hash() {
